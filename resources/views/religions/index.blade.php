        ...
        <div class="col-md-12">
            <table class="table table-sm table-hover border" id="mytable">
                ...
                @foreach($religions as $idx => $religion)
                    <tr>
                        <td><input type="checkbox" name="singlechecks" class="form-check-input singlechecks" value="{{$religion->id}}"></td>
                        <td>{{++$idx}}</td>
                        <td>{{$religion->name}}</td>
                        <td>{{$religion->status->name}}</td>
                        <td>{{$religion->user->name}}</td>
                        <td>{{$religion->created_at->format("d M Y")}} {{$religion->created_at->diffForHumans()}}</td>
                        ...
                        <td>
                            ...
                            <form action="{{route('religions.destroy',$religion->id)}}" method="post" id="formdelete-{{$idx}}">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
