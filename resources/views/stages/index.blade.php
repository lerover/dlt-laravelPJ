        ...
        <div class="col-md-12">
            <table class="table table-sm table-hover border" id="mytable">
                ...
                @foreach($stages as $idx => $stage)
                    <tr>
                        <td><input type="checkbox" name="singlechecks" class="form-check-input singlechecks" value="{{$stage->id}}"></td>
                        <td>{{++$idx}}</td>
                        <td>{{$stage->name}}</td>
                        <td>{{$stage->status->name}}</td>
                        <td>{{$stage->user->name}}</td>
                        <td>{{$stage->created_at->format("d M Y")}} {{$stage->created_at->diffForHumans()}}</td>
                        ...
                        <td>
                            ...
                            <form action="{{url('stages/'.$stage->id)}}" method="post" id="formdelete-{{$idx}}">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
